...
class Node(object):
    ...
    def calc_output(self):
        ...
        :return:
        '''
        output = reduce(lambda ret,
                               conn: ret + conn.upstream_node.output * conn.weight,
                        self.upstream, 0)
    ...
    def calc_hidden_layer_delta(self):
        ...
        downstream_delta = reduce(
            lambda ret,
                   conn: ret + conn.downstream_node.delta * conn.weight,
            self.downstream, 0.0)
        self.delta = self.output * (
                1 - self.output) * downstream_delta
    ...
    def __str__(self):
        ...
        node_str = '%u-%u: output: %f delta: %f' % (
            self.layer_index, self.node_index, self.output,
            self.delta)
...
class Network(object):
    ...
    def __init__(self, layers):
        ...
        for layer in range(layer_count - 1):
            ...
            for conn in connections:
                ...
                conn.downstream_node.append_upstream_connection(
                    conn)
                conn.upstream_node.append_downstream_connection(
                    conn)
    ...
    def calc_delta(self, label):
        ...
        for i in range(len(label)):
            output_nodes[i].calc_output_layer_delta(
                label[i])
