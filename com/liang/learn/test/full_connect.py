...
# 全连接层实现类
from com.liang.learn.CNN.full_connect_vector import \
    SigmoidActivator, evaluate
...
def transpose(args):
    return map(
        lambda arg: map(
            lambda line: np.array(line).reshape(len(line),
                                                1)
            , arg)
        , args
    )
...
class Normalizer(object):
    ...
    def norm(self, number):
        data = map(lambda m: 0.9 if number & m else 0.1,
                   self.mask)
        return np.array(data).reshape(8, 1)

    def denorm(self, vec):
        binary = map(lambda i: 1 if i > 0.5 else 0,
                     vec[:, 0])
...
def correct_ratio(network):
    ...
    for i in range(256):
        if normalizer.denorm(
                network.predict(normalizer.norm(i))) == i:
            correct += 1.0
